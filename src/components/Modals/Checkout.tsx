...
const Checkout = ({
  ...
  return (
    <form className="bg-white px-6 md:p-10 lg:flex md:max-w-[1400px] mx-auto lg:space-x-10 xl:space-x-20 pt-6 md:pt-10 h-full">
      ...
      <div className="justify-between w-full lg:w-4/12 py-6 lg:pt-0">
        ...
        <div className="flex justify-between text-md font-semibold pt-2 md:pt-6">
          <p>Pay</p>
          <p> {`${property?.currency}`} {total?.toLocaleString()}</p>
        </div>

        <div className='flex space-x-2 pt-6 text-sm'>
          <Checkbox isChecked={checked} onChange={() => setChecked(!checked) } />
          <p>I have read and accepted the booking policy</p>
        </div>

        <div className="space-x-6 pt-6 flex justify-between text-sm">
          <Button
            disabled={!checked}
