  ...
  return (
    <div className="lato">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 w-full gap-10 pt-8">
        <StatCard title="Total Impressions" amount={isLoading ? <Spinner/> : data?.total_impressions} />
        <StatCard title="Successful Bookings" amount={isLoading ? <Spinner/> :  data?.successful_bookings} />
        <StatCard title="Conversion rate" amount={isLoading ? <Spinner/> : "0"} />
        <StatCard title="Total Revenue" amount={isLoading ? <Spinner/> : data?.revenue} />
      </div>
